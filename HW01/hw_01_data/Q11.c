...
void SolutionsFinder(int board[N], int column, bool used[3][DIAG]);
...
void SolutionsFinder(int board[N], int column, bool used[3][DIAG])
{
	...
	for (int row = 0; row < N; row++) {
		...
		SolutionsFinder(board, column + 1, used);
...
void UsedMark(int used[3][DIAG], bool mark, int r, int c)
{
	used[0][r] = mark;
	used[1][r + c] = mark;
